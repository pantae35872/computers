...
public class ComputerMenu extends AbstractContainerMenu {
    ...
    public ComputerMenu(int id, Inventory inv, BlockPos blockPos) {
        this(id, inv,
                Objects.requireNonNull(inv.player.level().getBlockEntity(blockPos)),
                new SimpleContainerData(2));
    }

    public ComputerMenu(int id, Inventory inv, BlockEntity blockEntity, ContainerData simpleContainerData) {
        super(ModMenuTypes.COMPUTER.get(), id);
        checkContainerSize(inv, 2);
        this.blockEntity = (ComputerBlockEntity) blockEntity;
    ...
    public boolean stillValid(@NotNull Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.COMPUTER.get());
    }
